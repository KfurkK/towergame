...
public class Main extends Application {
	...
	private static final Color[] YELLOW_TONES = { Color.web("FFCF50"), Color.web("FBC518") };
	...
	private void scheduleWaves(int level) {
		...
		for (int i = 0; i < waveData.length; i++) {
			...
			master.getKeyFrames().add(new KeyFrame(Duration.seconds(delay), e -> spawnWave(count, rate)));
	...
	private Button createGameButton(String text, double x, double y) {
		...
		button.setStyle("-fx-font-size: 16px;" + "-fx-background-color: #FBD18B;" + "-fx-border-radius: 12;"
				+ "-fx-background-radius: 12;" + "-fx-text-fill: black;");
		button.setLayoutX(x);
		button.setLayoutY(y);
		return button;
	}
	...
	private Scene getGameScene(StackPane gameRoot) throws FileNotFoundException {
		...
		pathCoordinates = tools
				.readCoordinates("C:\\Users\\asara\\OneDrive\\Masaüstü\\TermProject\\levels\\level2.txt");
	...
	private void updatePathDebugInfo() {
		...
		for (int i = 0; i < Math.min(5, pathCoordinates.size()); i++) {
			pathInfo.append("[").append(pathCoordinates.get(i)[0]).append(",").append(pathCoordinates.get(i)[1])
					.append("] ");
		}
	...
	private VBox createHudPanel() {
		...
		for (Button b : new Button[] { singleShot, laser, tripleShot, missile }) {
			...
			b.setPrefWidth(150);
			b.setPrefHeight(90);
			b.setStyle("-fx-font-size: 16px;" + "-fx-background-color: #FBD18B;" + "-fx-border-radius: 12;"
					+ "-fx-background-radius: 12;" + "-fx-text-fill: black;");
		}
	...
	public void setupTowerPlacement() {
		...
		gameOverlay.setOnMouseClicked(e -> {
			if (draggingTower && selectedTower != null) {
				...
				if (clickX < offsetX || clickY < offsetY || col < 0 || col >= GRID_SIZE || row < 0
						|| row >= GRID_SIZE) {
					invalid = true;
				}
				...
				placed.getNode().setOnMouseReleased(ev -> {
					if (draggingTower && selectedTower == placed) {
						...
						if (row1 < 0 || row1 >= GRID_SIZE || col1 < 0 || col1 >= GRID_SIZE) {
							...
							placedTowerCells.removeIf(
									p -> p[0] == placed.getGridPosition()[0] && p[1] == placed.getGridPosition()[1]);
							Game.removeTower(placed);
							// seçimi temizle
							selectedTower = null;
							selectedTowerType = 0;
						...
						if (!invalidPlacement) {
							for (int[] p : placedTowerCells) {
								if (p[0] == row1 && p[1] == col1 && !(p[0] == placed.getGridPosition()[0]
						...
						if (invalidPlacement) {
							...
							double origX = offsetX + placed.getGridPosition()[1] * gridUnit + TILE_SIZE / 2;
							double origY = offsetY + placed.getGridPosition()[0] * gridUnit + TILE_SIZE / 2;
			...
			if (selectedTowerType < 1 || selectedTowerType > 4)
				return;
			...
			if (tower != null) {
				...
				tower.getNode().setOnMouseReleased(ev -> {
					...
					placedTowerCells
							.removeIf(p -> p[0] == tower.getGridPosition()[0] && p[1] == tower.getGridPosition()[1]);
	...
	private static Button getStartButton() {
		...
		startButton.setStyle("-fx-font-size: 32px;" + "-fx-background-color: #c29b57;" + "-fx-text-fill: black;"
	...
	private static Button getloseButton() {
		...
		loseButton.setStyle("-fx-font-size: 32px;" + "-fx-background-color: #c29b57;" + "-fx-text-fill: black;"
				+ "-fx-background-radius: 40;" + "-fx-border-radius: 40;");
		return loseButton;
	}
}